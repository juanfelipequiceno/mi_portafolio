...
public final class ConsultarHabitacion extends javax.swing.JFrame {
    ...
    public ConsultarHabitacion(InterFactura ventana) {
    ...
    public void actualizarHabitacion(){
        ...
        for (int i = 0; i < this.listaHabitaciones.length ; i++) {
            if(this.listaHabitaciones[i] !=  null){
                if(this.listaHabitaciones[i].getEstado().equals("Activo")){
    ...
    private void tablaHabitacionMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tablaHabitacionMouseClicked
        ...
        if (this.listaHabitaciones[filaSeleccionada].getEstado().equals("Activo" ) == true) {
            ventana.actualizarHabitacion(this.listaHabitaciones[filaSeleccionada]);
            ventana.setVisible(true);
            dispose();
        } else {
            Alert alerta = new Alert("Datos Inválidos", "El Estadeo De La Habitaion Es Inactivo.", "error");
        }
